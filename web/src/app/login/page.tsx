...
export default function LoginPage() {
  ...
  useEffect(() => {
    if (connected && publicKey) {
      ...
      login('admin', publicKey.toString(), 0); 
      router.push('/dashboard/admin');
    }
  }, [connected, publicKey, login, router]);

  const handleGoogleLogin = () => {
    setIsLoadingAuth(true);
    const toastId = toast.loading('Authenticating with Google...');
    setTimeout(() => {
      login('admin', 'Google_User_' + Math.floor(Math.random() * 1000), 0);
  ...
  const handleVerifyOtp = () => {
    ...
    setIsLoadingAuth(true);
    const toastId = toast.loading('Verifying code and generating secure wallet...');
    setTimeout(() => {
      login('admin', 'Email_Wallet_' + email.split('@')[0], 0);
    ...
    <div className="min-h-screen flex items-center justify-center bg-zinc-50 dark:bg-[#050505] p-6 selection:bg-blue-500/30 dark:selection:bg-[#00ffcc]/30">
      ...
      <div className="w-full max-w-md bg-white dark:bg-[#080808] border border-zinc-200 dark:border-zinc-800 p-8 sm:p-10 rounded-2xl shadow-xl relative overflow-hidden transition-all duration-500">
        ...
        <div className="flex flex-col items-center justify-center mb-8 text-center">
          <div className="size-12 bg-blue-50 dark:bg-zinc-900 border border-blue-100 dark:border-zinc-800 rounded-xl flex items-center justify-center mb-6">
          ...
          <h2 className="text-2xl font-bold tracking-tight text-zinc-900 dark:text-white mb-2">
            {isOtpSent ? 'Check your email' : 'Welcome to Axeon'}
          </h2>
          <p className="text-sm text-zinc-500">
            {isOtpSent 
              ? `We sent a 6-digit verification code to ${email}` 
              : 'Connect your wallet or use your email to enter the protocol.'}
          </p>
        </div>
        ...
        {isOtpSent ? (
          <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="flex justify-between gap-2">
              {otp.map((digit, index) => (
                <input
                  ...
                  // FIX: Menambahkan aria-label dan title untuk aksesibilitas form
                  aria-label={`Digit ${index + 1} of verification code`}
                  title={`Digit ${index + 1}`}
                  className="w-12 h-14 text-center text-xl font-bold bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl focus:outline-none focus:border-blue-500 dark:focus:border-[#00ffcc] text-zinc-900 dark:text-white transition-colors"
                />
              ))}
            </div>
            ...
            <button 
              ...
              disabled={isLoadingAuth || otp.join('').length < 6}
              className="w-full bg-zinc-900 dark:bg-white text-white dark:text-black font-bold text-sm uppercase tracking-widest rounded-xl h-12 hover:bg-blue-600 dark:hover:bg-[#00ffcc] transition-colors disabled:opacity-50 flex items-center justify-center"
            >
            ...
            <button 
              ...
              className="w-full text-xs font-mono text-zinc-500 hover:text-zinc-900 dark:hover:text-white transition-colors text-center"
            >
          ...
          <div className="space-y-6 animate-in fade-in zoom-in-95 duration-500">
            ...
            <form onSubmit={handleSendCode} className="space-y-3">
              ...
              <input 
                ...
                required
                // FIX: Menambahkan aria-label untuk aksesibilitas form
                aria-label="Email address"
                className="w-full bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 text-zinc-900 dark:text-white placeholder:text-zinc-400 text-sm rounded-xl h-12 px-4 focus:outline-none focus:border-blue-500 dark:focus:border-[#00ffcc] transition-colors"
              />
              <button 
                type="submit"
                disabled={isLoadingAuth || !email}
                className="w-full bg-zinc-900 dark:bg-white text-white dark:text-black font-bold text-xs font-mono uppercase tracking-widest rounded-xl h-12 hover:bg-blue-600 dark:hover:bg-[#00ffcc] transition-colors disabled:opacity-50"
              >
            ...
            <button 
              ...
              className="w-full flex items-center justify-center gap-3 bg-white dark:bg-[#080808] border border-zinc-200 dark:border-zinc-800 text-zinc-700 dark:text-zinc-300 font-bold text-sm rounded-xl h-12 hover:bg-zinc-50 dark:hover:bg-zinc-900 transition-colors disabled:opacity-50"
            >
            ...
            <div className="flex justify-center w-full axeon-wallet-wrapper">
              {/* FIX: Mengubah sintaks !class menjadi class! sesuai aturan Tailwind V4 */}
